...
class CenterJpaPersistenceAdapterIT {
  ...
  @Mock private CenterJpaPersistenceRepository repository;
  ...
  void create_shouldCreateEntitySaveAndReturnDomain() {
    ...
    // Then
    verify(mapper).update(any(CenterEntity.class), eq(center));
    verify(repository).save(any(CenterEntity.class));
  ...
  void read_shouldReturnEmpty_whenEntityNotFound() {
    ...
    // Then
    verify(repository).findById(id);
    verify(mapper, never()).toDomain(any(CenterEntity.class));
  ...
  void update_shouldFindEntityUpdateAndReturnDomain() {
    ...
    // Then
    verify(repository).findById(id);
    verify(mapper).update(centerEntity, center);
    verify(mapper).toDomain(centerEntity);
    assertNotNull(result);
  }
  ...
  void update_shouldThrowException_whenEntityNotFound() {
    ...
    verify(repository).findById(id);
    verify(mapper, never()).update(any(CenterEntity.class), any(Center.class));
  }
