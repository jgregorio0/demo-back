...
import dev.jgregorio.demo.back.infrastructure.api.center.search.CenterSearchRequest;
import dev.jgregorio.demo.back.infrastructure.api.center.search.CenterSearchResponse;
...
class SearchCenterControllerTest extends ApiPresentationTestHelper {
  ...
  void search_shouldFindCenter_whenRequestIsValid() throws Exception {
    ...
    verify(searchMapper).fromRequest(any(CenterSearchRequest.class));
    verify(centerService).search(centerSearch, pageable);
    verify(searchMapper).toResponse(center);
  }
  ...
  void search_shouldReturnEmptyPage_whenNoCentersFound() throws Exception {
    ...
    verify(searchMapper).fromRequest(any(CenterSearchRequest.class));
    verify(centerService).search(centerSearch, pageable);
    verify(searchMapper, never()).toResponse(any(Center.class));
  }
  ...
  void search_shouldReturnMultipleCenters_whenMultipleResultsFound() throws Exception {
    ...
    verify(searchMapper).fromRequest(any(CenterSearchRequest.class));
    verify(centerService).search(centerSearch, pageable);
    verify(searchMapper).toResponse(center1);
  ...
  void search_shouldHandlePagination_whenPageParametersProvided() throws Exception {
    ...
    verify(searchMapper).fromRequest(any(CenterSearchRequest.class));
    verify(centerService).search(eq(centerSearch), any(Pageable.class));
  }
