...
class CenterSpecificationTest {
  ...
  void search_whenAddressIsProvided_shouldCreateAddressPredicate() {
    ...
    when(cb.lower(addressPath)).thenReturn(lowerAddressPath);
    ...
    assertNotNull(result);
    assertEquals(finalPredicate, result);
    verify(cb).lower(addressPath);
    verify(hcb).collate(lowerAddressPath, "BINARY_AI");
    verify(cb).like(collatedPath, "%some address%");
    verify(cb).and(likePredicate);
  }
  ...
  void search_whenPostalCodeIsProvided_shouldCreatePostalCodePredicate() {
    ...
    when(cb.lower(postalCodePath)).thenReturn(lowerPostalCodePath);
    ...
    // Then
    verify(cb).lower(postalCodePath);
    verify(hcb).collate(lowerPostalCodePath, "BINARY_AI");
    verify(cb).like(collatedPath, "%12345%");
    verify(cb).and(likePredicate);
  }
}
